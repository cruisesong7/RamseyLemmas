...
lemma Ineq₉_helper {G : SimpleGraph (Fin 27)} [DecidableRel G.Adj] (hxy: G.isXYGraph 3 8) : G.edgeFinset.card ≥ 80 := by
  have h1 := Prop₄ hxy
  simp [-Set.toFinset_card] at h1
  have σub : σ_G hxy < 4 := by
  ...
  have bilinearlb := σ_helper hxy σub (λ d ↦ (e 3 7 (26 - vᵢ 2 7 d - 1) + (vᵢ 2 7 d)^2) * (G.sᵢ 2 7 d)) (by simp only [Finset.filter_eq_empty_iff]; intros; apply sᵢ_vanish hxy _ 4; assumption; simp [R37])
  have vertexlb := σ_helper hxy σub (λ d ↦  1 * (G.sᵢ 2 7 d)) (by simp only [Finset.filter_eq_empty_iff]; intros; apply sᵢ_vanish hxy _ 4; assumption; simp [R37])
  have degreelb := σ_helper hxy σub (λ d ↦ (vᵢ 2 7 d) * (G.sᵢ 2 7 d)) (by simp only [Finset.filter_eq_empty_iff]; intros; apply sᵢ_vanish hxy _ 4; assumption; simp [R37])
